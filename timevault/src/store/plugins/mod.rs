...
pub trait FormatPlugin: Send + Sync + 'static {
    ...
    fn valid_prefix_len(&self, _file: &mut dyn ReadSeek) -> io::Result<Option<u64>> {
        Ok(None)
    }
    fn dedup_eq(&self, _a: &[u8], _b: &[u8]) -> bool {
        false
    }
}
...
pub fn resolve_plugin(name: &str) -> Result<Arc<dyn FormatPlugin>> {
    match name {
        ...
        other => Err(crate::errors::TvError::Io(std::io::Error::new(std::io::ErrorKind::InvalidInput, format!("unknown plugin: {}", other)))),
    }
}

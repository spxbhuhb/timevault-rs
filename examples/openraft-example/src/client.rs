...
pub struct ExampleClient {
    ...
    pub leader: Arc<Mutex<(TvrNodeId, String)>>,
...
impl ExampleClient {
    ...
    pub async fn init(&self) -> Result<(), typ::RPCError<typ::InitializeError>> {
        self.do_send_rpc_to_leader("init", Some(&Vec::<(TvrNodeId, String)>::new())).await
    }
    ...
    /// See [`RaftMetrics`].
    pub async fn metrics(&self) -> Result<RaftMetrics<TvrNodeId, BasicNode>, typ::RPCError> {
